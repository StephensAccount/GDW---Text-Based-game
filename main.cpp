...
bool player1Turn = true; // Checks if it is player 1's turn
bool player2Turn = false; // Checks if it is player 2's turn
...
void jabAttack()
{
	...
	int enemyPosX = adventurerList[0][1] + tempX; // Need to change the [0] to the turn of the adventurer
	int enemyPosY = adventurerList[0][2] + tempY; // Need to change the [0] to the turn of the adventurer
...
void rangedAttack() {
	...
	if (finalCheck == 1) {
		...
		if (dirInput == 1) {
			...
			int tempX = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			int tempY = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			for (int i = tempY + 1; i < 6; i++) { // Goes through the Y axis, + 1 is used to prevent the player shooting himself
				...
				if (hit == false) { // Skips the check if the Arrow has already hit something
					for (int j = 0; j < 4; j++) { // Checks every Adventurer's position
						if ((adventurerList[j][2] == i) && (adventurerList[0][1] == adventurerList[j][1])) { // Replace the [0] with the turn of the Adventurer, 
		...
		if (dirInput == 2) {
			...
			if (dirInput == 1) {
				...
				int tempX = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
				int tempY = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
				for (int i = tempY - 1; i >= 0; i--) { // Goes through the -Y axis, - 1 is used to prevent the player shooting himself
					if (map[tempX][i] == '#') { // Checks for obstacles
						cout << "\n!!The arrow hit an obstacle and was stopped!!\n";
						hit = true;
					}
		...
		if (dirInput == 3) {
			...
			int tempX = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			int tempY = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			for (int i = tempX - 1; i >= 0; i--) { // Goes through the -X axis, - 1 is used to prevent the player shooting himself
				if (map[i][tempY] == '#') { // Checks for obstacles
					cout << "\n!!The arrow hit an obstacle and was stopped!!\n";
					hit = true;
				}
				if (hit == false) { // Skips the check if the Arrow has already hit something
					for (int j = 0; j < 4; j++) { // Checks every Adventurer's position
						if ((adventurerList[j][1] == i) && (adventurerList[0][2] == adventurerList[j][2])) { // Replace the [0] with the turn of the Adventurer, 
																											 //the second condition is used to check the y axis
							cout << "\nYou attacked the enemy for 4 DMG!\n";
							adventurerList[j][0] = adventurerList[j][0] - 4; // Inflict 3 damage
							if (adventurerList[j][0] < 0) {
								adventurerList[j][0] = 0; // Prevents the health from being in the negative, as it will affect the win
		...
		if (dirInput == 4) {
			...
			int tempX = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			int tempY = adventurerList[0][1]; // Replace the [0] with the turn of the Adventurer
			for (int i = tempX + 1; i < 8; i++) { // Goes through the X axis, + 1 is used to prevent the player shooting himself
				if (map[i][tempY] == '#') { // Checks for obstacles
					cout << "\n!!The arrow hit an obstacle and was stopped!!\n";
					hit = true;
				}
...
void swingAttack() {
	...
	cin >> finalCheck;
	if (finalCheck == 1) {
		// Insert code for swing attack here
...
void AdventurerItems() { // Need to input the adventurer number here!
	if (player1Turn == true) {
		if (player1PotionCounter > 0) {
			adventurerList[0][0] = adventurerList[0][0] + 2; // First slots needs to be the Adventurer number
...
void move()
{
	...
	if (moveInput == 1)
	{
		...
		if (map[adventurerList[0][1] + newX][adventurerList[0][2] + newY] != 'E' && map[adventurerList[0][1] + newX][adventurerList[0][2]+ newY] != '#') 
		{
			map[adventurerList[0][1]][adventurerList[0][2]] = '-';
			adventurerList[0][1] += newX;
			adventurerList[0][2] += newY;
		}
...
int main()
{
	do {
		...
		playerTurn();
		repeatTurn();
	} while ((player1TotalHealth > 0) && (player2TotalHealth > 0)); // Checks if the total health pool of the player have not reached 0 yet
	pickingWinners();
}
